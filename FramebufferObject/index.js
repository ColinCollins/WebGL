...
function main() {
    ...
    function onComplete () {
        ...
        draw(gl, canvas, fbo, angle, texture, viewProjMatrix, viewProjMatrixFBO, indices);
    }
}
...
function draw (gl, canvas, fbo, angle, texture, viewProjMatrix, viewProjMatrixFBO, indices) {
    ...
    // 绘制立方体
    drawTextureCube(gl, angle, texture, viewProjMatrixFBO, indices);
    // 下面是实际 canvas 绘制
    ...
    gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT);
    // 传入 texture piexl 数据
    drawTexturePlane(gl, fbo.texture, viewProjMatrix);
}

function drawTextureCube (gl, angle, texture, viewProjMatrixFBO, indices) {

    let a_Position = getAttribProp(gl, 'a_Position');
    let a_TexCoord0 = getAttribProp(gl, 'a_TexCoord0');
    ...
    gl.uniformMatrix4fv(u_ProjMatrix, false, viewProjMatrixFBO);

    gl.drawElements(gl.TRIANGLES, indices.length, gl.UNSIGNED_BYTE, 0);
}

function drawTexturePlane (gl, texture, viewProjMatrix) {
...
function initNewTexture (gl, u_Sampler, image, num) {
    ...
    gl.texParameteri(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, image);
